...
export class InitialState extends State {
	...
	enterState(bee) {
		bee.topSpeed = 0;
        //console.log("Entering Initial State, Checking for Bear Intrusion");
	}
	...
	updateState(bee, controller, gameMap, bear) {
        ...
        && (Math.abs(bee.location.z - bear.location.z) < gameMap.tileSize/4) ) {
			...
			setTimeout(() => {
				...
				//console.log("Found Bear intrusion");
...
export class TransitionState extends State {
	...
	updateState(bee, controller, gameMap, bear) {
		...
		if (bee.beehive.count == 0){
			if (bee.beehive.isPickedUp == true) bee.beehive.isPickedUp = false; 
			let node = gameMap.getNodeFromHalton();
			bee.beehive.location = gameMap.localize(node);
			//console.log("beehive position", bee.beehive.location);
			bee.beehive.count = 0;
		}
